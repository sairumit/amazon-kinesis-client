...
public class DynamoDBLeaseRefresher implements LeaseRefresher {
	...
	public boolean createLeaseTableIfNotExists(@NonNull final Long readCapacity, @NonNull final Long writeCapacity)
		...
		// ProvisionedThroughput.builder().readCapacityUnits(readCapacity)
		// .writeCapacityUnits(writeCapacity).build();
		// CreateTableRequest request =
		// CreateTableRequest.builder().tableName(table).keySchema(serializer.getKeySchema())
		...
		CreateTableRequest request = CreateTableRequest.builder().tableName(table).keySchema(serializer.getKeySchema())
				.attributeDefinitions(serializer.getAttributeDefinitions()).billingMode(BillingMode.PAY_PER_REQUEST)
				.build();
	 ...
	 *
	 * @param limit
	 *            number of items to consider at a time - used by integration tests
	 *            to force paging.
	 * @return list of leases
	 * @throws InvalidStateException
	 *             if table does not exist
	 * @throws DependencyException
	 *             if DynamoDB scan fail in an unexpected way
	 * @throws ProvisionedThroughputException
	 *             if DynamoDB scan fail due to exceeded capacity
	 */
	...
	public boolean renewLease(@NonNull final Lease lease)
		...
		try {
			...
			// If we had a spurious retry during the Dynamo update, then this conditional
			// PUT failure
			// might be incorrect. So, we get the item straight away and check if the lease
			// owner + lease
	...
	/*
	 * This method contains boilerplate exception handling - it throws or returns
	 * something to be thrown. The inconsistency there exists to satisfy the
	 * compiler when this method is used at the end of non-void methods.
	 */
